...
def process_vendors(
    ...
    for vendor_id, vendor in new_vendors.items():
        if old_vendors.get(vendor_id) is None:
            logging.info(
                f"Vendor ID {vendor_id} is new. Skipping building inventory logs"
            )
            continue
...
def get_coordinates_of_affected_vendors(
    ...
    new_vendor_id_to_info: dict,
) -> dict:
    updated_vendor_id_to_info = {}

    vendor_added_or_location_logs = [
        log
        for log in vendor_logs
        if log["event_type"] in CONST_VENDOR_EVENT_TYPES_FOR_UPDATES
    ]  # Vendor IDs whose coordinates need to be fetched

    vendor_ids_added_or_location = {
        log["vendor_id"] for log in vendor_added_or_location_logs
    }  # previously: - CONST_EXCLUDED_VENDOR_IDS. Not sure if this is still needed after removing the excluded vendors during fetch

    spatially_unaffected_vendor_ids = (
        new_vendor_id_to_info.keys()
        - vendor_ids_added_or_location
        - CONST_EXCLUDED_VENDOR_IDS
    )  # Vendor IDs whose coordinates do not need to be fetched
    ...
    if len(vendor_ids_added_or_location) != 0:
        geolocator = Nominatim(
            user_agent="homeless-vendor-geocoder/1.0 aboodcr@outlook.com", timeout=10
        )
        ...
        for vendor_id in vendor_ids_added_or_location:
            ...
            try:
                time.sleep(5)
                coordinates = map_address_to_coordinates(
                    geolocator, street, postalcode, city
                )
